class Basesort {
    ...
    create(){      
        ...
        this.cop = [...this.array];
        const spacing = (180 - this.magrin*2)/this.nElems;
        this.moves=[];
        for(let i=0; i<this.array.length; i++){
            const x = (i*spacing+spacing/2 + this.magrin)*4;
            const y = this.mycanvasheight - (this.magrin*2) - i *7;
            let width1 = 0;
            if(this.nElems == 10){ width1 = (spacing-5)*4.5;}
            else if (this.nElems == 20){ width1 = (spacing-4)*4.5;}
            else{ width1 = (spacing- 3)*4.5;}
            const width = width1;
    ...
    sort(){
    let selectElement = document.querySelector(this.namebutton1);
    let selectvalue = selectElement.value;
    let selectspeed = document.querySelector(this.namebutton2);
    let speedvalue = selectspeed.value;
    if(selectvalue != this.nElems){
        this.create();
    }
    else if(speedvalue != this.speed){
